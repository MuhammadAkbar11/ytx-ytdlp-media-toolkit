...
describe('ConfigCommand', () => {
  ...
  test('should set config value with type validation', () => {
    const mockConfigService = {
      get: (key: string) => {
        if (key === 'useDownloadArchive') return false;
        return undefined;
      },
    ...
    command.execute(['set', 'useDownloadArchive', 'true']);

    console.log = originalLog;

    expect(logs.length).toBe(1);
    expect(logs[0]).toBe('Updated useDownloadArchive to true');
  });
  ...
  test('should fail on type mismatch', () => {
    ...
    command.execute(['set', 'useDownloadArchive', 'not-a-boolean']);
