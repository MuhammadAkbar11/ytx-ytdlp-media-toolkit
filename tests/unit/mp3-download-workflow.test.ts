...
describe('Mp3DownloadWorkflow', () => {
  const makeWorkflow = (exitCode: number, stderr: string = '') => {
    const validator = new ProfileValidator();
    const builder = new ArgumentBuilder();
    const eventStream = new EventStream();
    const runner: ProcessRunner = {
      run: async (_cmd: string, _args: string[], options?: any) => {
        if (options?.onStdout && exitCode === 0) {
          options.onStdout(
    ...
    return {
      workflow: new Mp3DownloadWorkflow(
        validator,
        builder,
        runner,
        eventStream
  ...
  test('should execute successfully and emit events', async () => {
    const { workflow, eventStream } = makeWorkflow(0);
    const events: any[] = [];
  ...
  test('should emit failed event on process failure', async () => {
    const { workflow, eventStream } = makeWorkflow(1, 'error');
    const events: any[] = [];
    eventStream.subscribe((e: any) => {
      if (e.type !== 'debug') events.push(e);
    });
